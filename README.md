...
| 最新已上传开发版本 | `1.0.39`（2026-09-15 上传成功，尚未代为提交审核或发布） |
| 最近云端修复 | 2026-09-13：打卡背景启停与草稿隔离修复已部署 |
| 默认分支 | `main` |
...
2026-09-13 已完成本轮客户 12 项反馈的回归验收，包含正式云端读写、开发者工具页面交互和图片生成。2026-09-15 追加修复资料页舟币不足提示：适配原生弹窗按钮长度限制，增加资料页“去分享”入口，返回资料页自动刷新余额，并同步修正舟币中心规则文案。已通过开发者工具上传为 `1.0.39`，后续以此版本验收、提交审核。上传成功不等于审核通过或正式发布。测试范围、上传记录与未覆盖项见 [9 月 13 日修复与验收记录](docs/verification-2026-09-13.md)。
