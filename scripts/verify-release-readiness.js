...
function checkLoginAndPaymentFixes() {
  ...
  const agreementWxml = read('pages/agreement/agreement.wxml')
  ...
  assert(loginWxml.includes('>手机号快捷登录</button>'), 'phone login button must use the review-compliant label')
  assert(!loginWxml.includes('微信'), 'login page must not contain WeChat-branded wording')
  assert(!/微信登录|微信官方能力|微信官方提供|微信授权/.test(`${agreementWxml}\n${privacyWxml}`), 'agreements must not use confusing WeChat login wording')
  assert(loginWxml.includes('/assets/images/logo.jpg'), 'login page must use the app brand logo')
